...
public class MapTask {
    public static void main(String[] args) {
        ...
        for (Map.Entry<String, String> b : firstTaskMap.entrySet()) {
            if (b.getValue() == city) a.add(b.getKey());
        }
        System.out.println(a + " первая задача");
        ...
        words.add("august");
        words.add("august");
        HashMap<String, Integer> vas = new HashMap<>();
        for (String q : words) {
            vas.put(q, vas.getOrDefault(q, 0) + 1);//непонятна работа getOrDefault
        }
        System.out.println(vas + " вторая задача");
        ...
        int from = 17;
        int to = 48;
        HashMap<String, Customer> FromTo = new HashMap<>();
        for (Map.Entry<String, Customer> b : customerMap.entrySet()) {
            if (b.getValue().getAge() >= from && b.getValue().getAge() < to) FromTo.put(b.getKey(), b.getValue());
        }
        System.out.println(FromTo + " задача №3");
